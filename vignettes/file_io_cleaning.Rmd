...
tail(crime, 20)
```
I like using base subsetting for these problems since usually the reason you're removing rows at the bottom is because a tool will output this information at the bottom of an output file. In case you rerun the tool later with more data and the length of the file changes, you can't rely on reading in a set number of lines. If the tool always outputs the same number of lines, it's safer to just drop the last `n` lines from the data after reading. We'll do this here. 
```{r}
keep <- 1:(nrow(crime) - 8)
crime <- crime[keep,]
```
...
R, so it also is a bad character for colnames. We'll just delete it.

There's also some spaces in the colnames (look carefully!). We don't want those either, so change them to "_".
```{r}
names(crime) %>%
  gsub("\n", "_", .) %>% 
  gsub("/", ".", .) %>% 
...
regex's. This is often true for other regex characters like '.' (match any character) which has to be
`\\.` in R. This might not make sense right now, but if you work more with
regex's later, keep this in mind!
