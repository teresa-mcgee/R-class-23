...
labelLines <- function(outer = FALSE, alpha = 1) {
  ...
  par(xpd = NA, lty = 3)
  if (outer) {
    par(col = col2alpha("darkgreen", alpha))
    p <- "oma"
    a <- par()$mar
  } else {
    par(col = col2alpha("darkred", alpha))
    p <- "mar"
    a <- rep(0, 4)
  }
  nline <- par()[[p]]
  ...
  mtext(txt, side, line, outer = outer)
  mtext(paste0("Side", 1:4), 1:4, -1, col = col2alpha("gray40", alpha))
  abline(h = line2user(0:nline[1] + a[1], 1))
  abline(v = line2user(0:nline[2] + a[2], 2))
  abline(h = line2user(0:nline[3] + a[3], 3))
  abline(v = line2user(0:nline[4] + a[4], 4))
}
