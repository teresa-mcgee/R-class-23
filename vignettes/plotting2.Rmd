...
```{r,fig.width=6,fig.height=1.5,fig.align='center'}
grayPal <- colorRampPalette(c("darkblue", "gray80", "darkred"), bias = 0.5)
palette(grayPal(30))
par(mar = rep(0.5, 4))
barplot(rep(1, 30), col = 1:30, ann = FALSE, axes = FALSE)
```

Note the *`bias`* parameter changes the palette center, which would be grey at the default *`bias`* value of 1. Creating custom palettes can be daunting. We suggest using the [RColorBrewer](https://cran.r-project.org/web/packages/RColorBrewer/index.html) and [colorspace](https://cran.r-project.org/web/packages/colorspace/) packages, which both provide functionality for creating visually appealing and colorblind-safe palettes. More information about the packages is available as a supplemental vignette in this package called "palettes". 
...
boxplot(iris$Sepal.Length ~ iris$Species, 
        col = c('magenta', 'orange', 'blue'), pch = 15, cex = 1.2)
