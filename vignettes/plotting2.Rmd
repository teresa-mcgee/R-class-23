...
The `legend()` function draws a legend or key to a plot in the traditional graphics system. Typically, the legned is drawn within the plot region, and located relative to the user coordinates. 

Now let's take a look at a simple example using the iris dataset:
...
plot(x = iris$Sepal.Length, y = iris$Petal.Length, 
     col = match(iris$Species, c("setosa", "versicolor", "virginica")),
     pch = 16, cex = 1.2)
legend(x = 4.5, y = 7, legend = c("setosa","versicolor","virginica"), 
       col = c(1:3), pch = 16, title = "Species")
```

Several arguments are used in the `legend()` function. You can type `?legend` for details in R documentation. Here, we will point out a few common ones.

The x and y coordinates are used to position the legend in the graph. The position of the legend can also be specified using the following keywords: "bottomright", "bottom", "bottomleft", "left", "topleft", "top", "topright", "right" and "center":

```{r, fig.width=6.5,fig.height=5.5, fig.align='center'}
plot(x = iris$Sepal.Length, y = iris$Petal.Length, 
     col = match(iris$Species, c("setosa", "versicolor", "virginica")), 
     pch = 16, cex = 1.2)
legend(x = 7, y = 3, legend = c("setosa","versicolor","virginica"), 
       col = c(1:3), pch = 16, title = "Species")
```

The *`fill`* argument indicates the colors to use for filling the legend boxes beside the legend text. *`border`* is the border color of the legend boxes, and is used only if *`fill`* is specified:

```{r, fig.width=6.5,fig.height=5.5, fig.align='center'}
boxplot(iris$Sepal.Length ~ iris$Species, col = c('magenta', 'orange', 'blue'), cex = 1.2)
legend(x = "topleft", legend = c("setosa","versicolor","virginica"), title = "Species",
       fill = c('magenta', 'orange', 'blue'), border = "green")
```

The *`ncol`* argument indicates the number of columns in the legend items (by default R uses 1, a vertical legend):
...
plot(x = iris$Sepal.Length, y = iris$Petal.Length, 
     ...
     pch = 16, cex = 1.2)
legend(x = 4.5, y = 7, legend = c("setosa","versicolor","virginica"), 
       col = c(1:3), ncol = 3, pch = 16, title = "Species")
```

You can specify lines in the legend by using *`lty`* instead of *`pch`*:

```{r, fig.width=6.5,fig.height=5.5, fig.align='center'}
plot(x = iris$Sepal.Length, type = "h", 
     col = match(iris$Species, c("setosa", "versicolor", "virginica")), 
     pch = 16, cex = 1.2)
legend(x = 4.5, y = 7, legend = c("setosa","versicolor","virginica"),
       col = c(1:3), lty = "solid", title = "Species")
```

The *`bty`* argument specifies the type of box to be drawn around the legend. Default is "o":
...
plot(x = iris$Sepal.Length, y = iris$Petal.Length, 
     ...
     pch = 16, cex = 1.2)
legend(x = 4.5, y = 7, legend = c("setosa","versicolor","virginica"), 
       col = c(1:3), pch = 16, title = "Species", bty = "n")
```

The alignment of the legend is analagous to text alignment. The *`xjust`* and *`yjust`* arguments determine how the legend is justified relative to the legend x and y locations. A value of 0 means left justified, 0.5 means centered and 1 means right justified. `legend()` returns a list with list components. *`rect`* is a list with components: *`w`*, *`h`* are positive numbers indicating the width and height of the legend. *`left`* and *`top`* are the xy coordinates of upper left corner of the box. Below is an example of how to right-justify the legend labels.

```{r, fig.width=6.5,fig.height=5.5, fig.align='center'}
## right-justifying a set of labels
plot(x = iris$Sepal.Length, y = iris$Petal.Length, 
     col = match(iris$Species, c("setosa", "versicolor", "virginica")), 
     pch = 16, cex = 1.2)
temp <- legend("bottomright", legend = c(" ", " "," "), pch = 16, 
       text.width = strwidth("1,000,000"), xjust = 1, yjust = 1,
       title = "Species", col = c(1:3))
#legend returns a list with list components
text(temp$rect$left + temp$rect$w, temp$text$y,
     c("setosa","versicolor","virginica"), pos = 2)
```
...
legend(legend = c("setosa","versicolor","virginica"), "topright", 
       inset = c(-0.3, 0), # specify distance(s) from the margins as a fraction of the plot region
       col = c(1:3), pch = 16, title = "Species")
```

### Organizing multiple plots

The graphics system provides two ways of organzing multiple plots in one device. The first way is the *`mfrow`* or *`mfcol`* parameter in `par`. The  *`mfrow`*/*`mfcol`* parameters take a length two vector defining the number of rows and columns (`[nrow, ncol]`) to create a grid of plots.
...
abline(0, 1, lwd = 40, col = "white")
points(1, 1, pch = "2", cex = 3)
```

Notice how changing the numbers in the matrix specifies the order of the drawing. In the example above the white line drawn during the second plot draws over the red box drawn in the first. 
...
Plots in R base plotting system are generated by calling successive R functions to "build" a plot. There are two stages in R base plotting: 1) creating a plot; 2) annotating an existing plot by adding lines, points, text, legends and so on. Below, we will briefly introduce several common plotting functions in base R:

1. Histogram

A histogram is a very common type of graphics and it plots the frequencies that data points occur in a certain range. Here is an example of a simple histogram made using the *hist()* function.
...
A scatterplot provides a way to visualize the relationship between two sets of numbers. In the 'Iris' dataset, we have four variables for observation: sepal length, sepal width, petal length, and petal width. We can look at the relationship between any of the four variables using *plot()*: function:

```{r, fig.width=6.5,fig.height=5.5, fig.align='center'}
data(iris)
head(iris)
plot(iris$Sepal.Length, iris$Sepal.Width, col = iris$Species, xlab = "Sepal length", ylab = "Sepal width", main = "Relationship between sepal length and sepal width")
```

What if we want to look at the relationships for all pairs of data points? You can use the *pair()* function to achieve this:
