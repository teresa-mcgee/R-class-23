...
In this section, we will briefly talk about the color basics in R, and then introduce a couple of popular and useful color packages that generate sets of colors. 

First, let's start off by looking at what colors are available in R? The `colors()` function (or `colours()`) provides the names of all 657 colors built-in colors in R.

```{r}
head(colors()) #or colours()
tail(colors()) 
```

This is probably the easiest way to specify a color in R is: using the color's name. An excellent reference for color names: http://research.stowers.org/mcm/efg/R/Color/Chart/ColorChart.pdf

What about other ways to specify colors?

You can also use the `rgb()` function (a Red-Green-Blue (RGB) triplet of intensities) to specify colors. The function `col2rgb()` can be used to see the RGB values. 

```{r}
col2rgb("red")
```

Instead of specifying colors by their color names (small positive integers and Crayola-style names), another way to specify a color is to give an RGB color specification by providing a string of the form __"#RRGGBB"__, where it starts with the leading `#` is by convention, and each of the pairs __RR__, __GG__, __BB__ consist of two hexadecimal digits giving a value in the range zero (00) to 255 (FF). The `RR`, `GG`, and `BB` refer to color intensity in the red, green, and blue channels, respectively. This is also called __"hexadecimal RGB color specification"__. In this specification, the color red is given as "#FF0000". 

The __RGB color models__ corresponds to color generation on a computer screen rather than human color perception, and it is virtually impossible for humans to control the perceptual properties of a color in this color space.

So, other than the __RGB color models__, are there any other color models that are good for perception? 

Yes, but we will introduce the __Hue-Chroma-Luminance (HCL)-based model__ later when we talk about the add-on packages: __RColorBrewer__ palettes and __colorspace__ (Note: ggplot2 uses this color model too). 
...
#barplot(rep(1,8), yaxt="n", col=c("black", "red","green3", "blue", "cyan","magenta", "yellow","gray"))
barplot(rep(1,8), yaxt="n", col=palette())
```

You can also recycle the colors if needed:

```{r, fig.width=6.5,fig.height=4.5, fig.align='center'}
# colors are "recycled" if necessary
barplot(rep(1,24),col=1:24, yaxt="n")
```

In summary, R colors can be specified by:

* color name

* hex constant (24-bit "True Color": 256^3 colors = 16,777,216 colors)

* index into palette

However, not everyone is good at picking colors (or has time to tinker with colors) for their figures. We can use the base R function `rainbow()` to generate sets of colors by specifying the number of colors:

```{r}
rainbow(12)
```

To visualize the 12 rainbow colors in a pie chart:
...
##conventional rainbow color wheel
wheel(rainbow(12))
```

Many add-on packages have been developed to help users generate sets of colors. For example:

The __RColorBrewer__ package [ColorBrewer](http://colorbrewer2.org/) developed by Cynthia Brewer, provides designed palettes of colors to use. 
...
Now, let's display the __ColorBrewer__ palettes:

```{r, fig.width=8,fig.height=10, fig.align='center'}
display.brewer.all()
```

There are three main classes of palettes in __RColorBrewer__:
...
We can also see a single __RColorBrewer__ palette by defining the name and size of the palette using the `brewer.pal()`. For example, the following code selects "Dark2" palette that contains eight colors.

```{r, fig.width=6.5,fig.height=4.5, fig.align='center'}
display.brewer.pal(n = 8, name = 'Dark2')
```

Another package to generate the sets of colors is the __colorspace__ package written by [Zeileis, et al](https://cran.r-project.org/web/packages/colorspace/vignettes/hcl-colors.pdf), which creates colors in a number of color spaces as well as functions to convert between color spaces. By contrast, the palettes in __RColorBrewer__ are fixed. 
...
#power control parameter determining how quickly or slowly chroma and/or luminance should be changed through the palette (1 = linear, 2 = quadratic, etc.).
```
...
Many R objects can take on different colors: points, lines, axes, text, legends, background. A simple example is given below using points:
...
__Some guidelines/principles about using colors in data visualization:__
...
* Jennifer Bryan et al, Stat 545A _Overview of using colors in R_. <https://www.stat.ubc.ca/~jenny/STAT545A/block14_colors.html>

* Earl F. Glynn (Stowers Institute for Medical Research)

* Edward Tufte. _The Visual Display of Quantitative Information and Envisioning Information_, Graphics Press, PO Box 430, Cheshire, CT 06410.

* Achim Zeileis, Kurt Hornik, Paul Murrell (2009). Escaping RGBland: Selecting Colors for Statistical Graphics. Computational Statistics & Data Analysis, 53(9), 3259-3270. <doi:10.1016/j.csda.2008.11.033>

* Ross Ihaka, Paul Murrell, Kurt Hornik, Jason C. Fisher, Achim Zeileis (2016). colorspace: Color Space Manipulation. R package version 1.3-2. <https://CRAN.R-project.org/package=colorspace>
